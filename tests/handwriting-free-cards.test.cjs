...
test('free-card capability check does not call unconfigured renderer',async()=>{
  ...
  let calls=0;
  await assert.rejects(client.requestRenderStack({login(){calls++;}},model.buildRenderPayload(insert(project(),{inkSticker:sticker}))),e=>e.code==='FUN_RENDERER_NOT_CONFIGURED');
  await assert.rejects(client.requestRenderStack({login(){calls++;}},model.buildRenderPayload(insert(project()))),e=>e.code==='FUN_RENDERER_NOT_CONFIGURED');
