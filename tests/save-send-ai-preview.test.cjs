...
test('cancelled AI response cannot overwrite a new batch and IDs are distinct',()=>{
  const calls=[],applied=[];
  ...
  page.showReviewTask=(task)=>applied.push(task.taskId);
  page.createProcessingTask([{fileId:'cloud://first'}]);
  page.onCancelProcess();page._cancelRequested=false;
  page.createProcessingTask([{fileId:'cloud://second'}]);
  assert.notEqual(calls[0].data.images[0].imageId,calls[1].data.images[0].imageId);
  calls[0].success({result:{taskId:'stale'}});
  assert.deepEqual(applied,[]);
  calls[1].success({result:{taskId:'current'}});
  assert.deepEqual(applied,['current']);page.onUnload();
});
