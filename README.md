...
**穿搭主链路已验收；阶段六“大字滑卡”代码已完成，已升级为粗颗粒马克笔白卡，待重新部署云托管渲染服务并做真机验收。定位已升级为叠图玩法生成器，玩法路线见 PLAYBOOK.md。**
...
- 底部三 Tab 导航：首页 / 记录 / 我的。
- 大字滑卡：输入 1–20 个字，默认生成粗颗粒马克笔白卡（轻微错落排版与原创贴纸）；1–2 个字自动补足引导卡，支持夜写与蓝紫主题、保存、微信滑动预览和本地记录。
...
大字滑卡不使用 AI 生成文字；它在云托管容器里使用内置的 OFL 1.1 授权 `LXGW Marker Gothic` 字体绘制粗颗粒马克笔 PNG，并在渲染前调用微信文本安全审核。视觉更新后必须重新部署该服务，部署步骤：
...
5. 用固定样例“今、你、开、心”真机生成：四张卡应都是粗黑马克笔笔触、存在克制的错落差异、每张各有一枚原创贴纸，并在 375px 宽预览中清晰可读。
6. 验证保存、微信滑动预览和历史记录；再生成一组应被拦截的文字，确认其停留在编辑页且不写入记录。安全审核不可用时也不得生成卡片。
...
│   ├── cloudhosting/
│   │   └── text-card-renderer/       # 云托管：内容审核 + 马克笔字卡 PNG 渲染
│   └── config/
