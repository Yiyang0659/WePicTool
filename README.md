...
- **趣味字画渲染底座（功能分支）**：独立 Node 20 云托管服务对 1080 场景执行白名单校验和二次内容审核，使用授权字体确定性输出 360 预览图或 1080 最终 PNG；尚未部署或接入候选页。
- **趣味字画输入与首页示例（功能分支）**：首页新增五张由 `render-demo.js` 用授权字体渲染的真实示例牌堆，滑到最后一张出现「把你的话也变成一叠」；输入页只提供一句话与组合表达标签，文案先经 `contentGuard` 二次审核再进入候选页（候选页属后续任务，开发中）。
...
│   │   ├── fun-text/                 # 趣味字画输入页：一句话 + 组合表达标签（开发中）
...
7. 重新编译小程序，选择图片后即可体验真实云存储上传、AI 分类抠图与安全审核链路；趣味字画目前的用户入口为首页五张真实示例与输入页，候选/编辑/结果页仍属后续开发任务，候选页接入前示例与输入会停在「开发中」状态。
