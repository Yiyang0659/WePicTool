...
WePicTool 是微信「合并发送 / 叠图」玩法生成器——分层云换装是当前旗舰玩法，AI 穿搭白底卡继续作为照片整理工具，玩法模板库是长期资产（详见 [`docs/product/PLAYBOOK.md`](docs/product/PLAYBOOK.md)）。当前功能分支已经跑通“内置模板 / 用户按部位上传 -> 四组编辑 -> 多叠预览 -> 按组保存 -> 回微信合并发送”的本地链路；真实微信发送效果仍需真机验收。
...
- **分层云换装旗舰入口**：首页展示头像/发型、上衣、下装、鞋子四叠独立滑动效果，并提供「直接试玩」和「上传自己的素材」两条路径。
- **内置模板与混合素材**：首套「抽象搞怪」模板包含四个部位各 3 张素材；任一组都可以混入用户图片或用系统素材补齐。
- **轻量换装编辑器**（`pages/dressup`）：支持四组素材上传、删除、左右排序、首图标记、每组 3 张阈值提示、本地草稿、按组保存和四步微信发送引导。
...
- **微信发送效果全屏预览**（`pages/preview`）：白色聊天外壳，54% 舞台比例等比适配，支持多组独立牌堆滑卡切换与纵向展开。
...
│   │   └── playRegistry.js           # 玩法注册表、四组定义与内置素材包
│   ├── pages/                        # 页面视图层
│   │   ├── index/                    # 首页 Tab：分层换装旗舰入口与 AI 工具入口
│   │   ├── dressup/                  # 分层换装编辑器：上传、混用、排序、保存与引导
...
│       ├── layeredDressup.js          # 分层换装项目模型、编辑规则与预览契约
│       └── previewLayout.js          # 预览页 54% 宽度手势舞台比例计算
├── docs/                             # [项目大脑] 治理体系与知识库
...
│   ├── layered-dressup.test.cjs      # 分层换装注册、项目规则、资源与页面入口测试
