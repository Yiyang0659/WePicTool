...
`codex/unified-stack-export` 已从上述共同基线建立。《统一叠图导出与顺序识别设计说明》已经用户确认，逐文件实施计划已保存并等待用户确认。设计纠正了旧文档中“以 01/02 文件名保证相册顺序”的不可实现承诺：微信保存接口不能指定目标文件名或系统相册排序；拟改为最终图片可见序号角标、串行保存和按角标选择引导。当前仍没有业务代码变化。
...
- `codex/unified-stack-export`：基于趣味字画共同基线建立；统一导出设计已确认，实施计划待确认，无业务代码变化。
...
1. 用户审阅并确认 `docs/superpowers/plans/2026-09-04-unified-stack-export.md`；确认后从 Task 1 开始实施。
