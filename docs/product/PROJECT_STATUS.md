...
- 阶段三白底卡片合成：已完成并经用户验收
- 阶段六大字滑卡：编辑、三套主题、云端内容审核、粗颗粒马克笔 PNG 渲染、结果/预览/记录闭环已实现

阶段六的本次视觉更新需要重新部署 `text-card-renderer`；验收固定样例为“今、你、开、心”，检查粗黑颗粒笔触、轻微错落、原创贴纸、375px 可读性，以及保存/预览/历史与审核拒绝行为。
...
| 阶段三：白底卡片生成 | 已完成 | DashScope qwen-image-2.0 抠图与前端 Canvas 白底合成 | 白底结果可保存，浅色衣物可辨认，单张失败不阻断 |
| 阶段四：结果页增强 | 未开始 | 微信叠图效果预览、单组滑动、保存引导增强 | 每组不足 3 张时清晰降级提示 |
| 阶段五：轻编辑与兜底 | 未开始 | 改分类（已完成）、删除、重做、排序、切比例、埋点 | 用户不用重新开始即可修正错误 |
| 阶段六：大字滑卡 | 待部署验收 | 粗颗粒马克笔 PNG 云托管渲染、文本审核、编辑/结果/微信预览/历史记录 | 正常文字可生成；违规和审核异常不得生成；iOS/Android 保存与预览可用 |
...
| 阻塞项 | 原因 | 解决方向 |
|--------|------|---------|
| 大字滑卡云托管未部署 | `TEXT_CARD_RENDERER_URL` 仍为空，编辑页会阻止最终生成 | 按 README 的“部署大字滑卡手写服务”重新部署，填入 HTTPS 地址并配置 request 合法域名 |
| 大字滑卡真机验收 | 云端服务部署后尚未验证保存、预览和审核拦截 | iOS/Android 各跑正常文字与违规文字一遍 |
...
### 2026-07-19（上线内容安全审核门禁）

- **做了什么：** 为用户上传图片和意见反馈文本接入微信云调用内容安全接口。`processOutfit` 在 Qwen 分类/抠图前对全部图片执行最多 2 张并发的同步审核；仅所有图片通过才继续 AI。新增 `contentGuard` 云函数，在反馈写入本地前审核文本。
- **拦截策略：** 违规（87014）返回 `CONTENT_UNSAFE`；接口异常、限流或超时返回 `SAFETY_UNAVAILABLE`；两者都不进入 AI、不跳转结果页。图片违规任务会尽力删除云存储源图，避免无效留存。修复了安全失败可能被通用异常逻辑降级为 mock 结果的风险。
- **部署动作：** 在微信开发者工具分别右键 `processOutfit` 与 `contentGuard`，选择“上传并部署：云端安装依赖”；部署后使用合规图片、违规图片与反馈文本各做一次真机验证，再把后台 UGC 声明改为“使用平台建议的内容安全 API”。
