...
- 阶段三白底卡片合成：已完成并经用户验收
- 阶段六大字滑卡：编辑、三套主题、云端内容审核与手写 PNG 渲染、结果/预览/记录闭环已实现
...
| 阶段六：大字滑卡 | 待部署验收 | 手写字体 PNG 云托管渲染、文本审核、编辑/结果/微信预览/历史记录 | 正常文字可生成；违规和审核异常不得生成；iOS/Android 保存与预览可用 |
...
| 阻塞项 | 原因 | 解决方向 |
|--------|------|---------|
| 大字滑卡云托管未部署 | `TEXT_CARD_RENDERER_URL` 仍为空，编辑页会阻止最终生成 | 按 README 的“部署大字滑卡手写服务”部署，填入 HTTPS 地址并配置 request 合法域名 |
