...
在不修改产品功能、不合并功能分支、不搬动既有产品文档的前提下，为 WePicTool 建立一套可持续执行的“规划 → 开发 → 验证 → 文档同步 → 提交”机制。Codex 与 Claude Code 的新会话都应只阅读各自入口规则和当前状态，即可理解项目现状、下一步和已知风险。
...
- 两个文件都只保留项目概述、命令、工具特定提示和指向 `docs/governance.md` 的链接；不得各自复制一套完整的文档同步规则。
- `docs/governance.md` 是唯一的流程规则正文，包含开发前、开发后、合并前规则，及新功能与纯修复的区分。
- 两个入口文件必须要求助手先阅读 `docs/governance.md`、`docs/current.md`、`docs/roadmap.md`，再开始涉及产品行为的工作。
- `CLAUDE.md` 中已有的项目架构、技术栈、命令和产品约束继续保留；仅替换其中过期的当前阶段说明和重复的流程约束，并把当前状态指向 `docs/current.md`。
...
1. Codex 读取 `AGENTS.md`，Claude Code 读取 `CLAUDE.md`；两个入口文件都要求继续读取 `docs/governance.md`、`docs/current.md`、`docs/roadmap.md` 和任务相关技术文档。
2. 确认本次任务只解决一个明确目标。
...
1. 验证治理文档及 `docs/iterations/` 存在，并验证 `AGENTS.md`、`CLAUDE.md` 都引用 `docs/governance.md`、`docs/current.md` 和 `docs/roadmap.md`。
