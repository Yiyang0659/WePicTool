...
2026-09-13 候选更新：新增鉴权POST `/render-capabilities`，与`/render-stack`必须指向同一版本；自由卡/混合风格客户端保存先核验能力，旧服务仍阻断。新版源码与当前边界见[审查交接](2026-09-13-review-handoff.md)。以下部署历史不能代替本次线上验证，勿直接用旧FULL发布脚本覆盖稳定服务。
