...
function planRuleCandidates(brief) {
  var strategyIds = selector.selectStrategyIds(brief.expressionKey, brief.variant);
  var candidates = strategyIds.map(function (strategyId) {
    ...
    var cards = strategy.buildCards(brief.sourceText, brief.variant).map(function (card, index) {
