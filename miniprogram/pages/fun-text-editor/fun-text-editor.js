...
function buildStyleControls(candidate, scene) {
  const pack = stylePacks.getStylePack((scene && scene.stylePackId) || (candidate && candidate.stylePackId));
...
Page({
  ...
  syncProject: function (project, cardIndex, selectedDecorationId) {
    project = funTextProject.repairFreeCardStyles(project);
  ...
  onSelectStylePack: function (event) {
    ...
    try {
      const updatedProject = funTextProject.updateStylePack(
        this.data.project,
        this.data.selectedCandidate.candidateId,
