...
async function processMatting(images, classifications, apiKey) {
  const mattedResults = {};
  ...
  for (let batchStart = 0; batchStart < mattingTasks.length; batchStart += CONCURRENCY) {
    ...
    for (let i = batchStart; i < batchEnd; i++) {
      ...
      batch.push(
        (async () => {
          try {
            const imageInput = await downloadImageAsBase64(task.image);
...
async function classifySingleImage(image, index, apiKey, retryCount) {
...
async function classifyImages(images, apiKey) {
  ...
  for (let batchStart = 0; batchStart < images.length; batchStart += CONCURRENCY) {
    ...
    for (let i = batchStart; i < batchEnd; i++) {
      batch.push(
        classifySingleImage(images[i], i, apiKey, 1).then(result => {
          results[i] = result;
        })
      );
    }
...
exports.main = async (event) => {
  ...
  const classifications = await classifyImages(normalizedImages, apiKey);
  console.log('分类结果:', JSON.stringify(classifications));

  // 阶段三：对 tops/bottoms/shoes 做抠图
  const mattedResults = await processMatting(normalizedImages, classifications, apiKey);
